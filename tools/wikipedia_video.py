...
logger.info("Importing: HumorousDiscussionGenerator, DiscussionFormat")
...
logger.info("Importing: AudioDurationEstimator")
...
def main():
    ...
    # Step 1.5: Download Wikipedia image if requested
    ...
    if args.include_image:
        with LoggedOperation(logger, "Wikipedia image download"):
            try:
                ...
                wikipedia_image_path = download_wikipedia_image(content)
                if wikipedia_image_path:
                    logger.info(f"Wikipedia image downloaded: {Path(wikipedia_image_path).name}")
                else:
                    logger.warning("No Wikipedia image available for this page")
            except Exception as e:
                logger.warning(f"Wikipedia image download failed: {e}")
                wikipedia_image_path = None
    ...
    with LoggedOperation(logger, "enhanced speech generation"):
        if args.engine == 'chattts':
            logger.info("Importing: ChatTTSSpeechGenerator, ChatTTSConfig")
            ...
            logger.info("Importing: CoquiSpeechGenerator, CoquiTTSConfig")
            ...
            with VideoClip(optimized_template) as video_clip:
                ...
                if wikipedia_image_path:
                    image_result = video_clip.add_wikipedia_image(
                        ...
                        duration=5.0,  # Display for first 5 seconds
                        fade_duration=0.5
                    )
                    if image_result['success']:
                        logger.info(f"Wikipedia image added to video")
                    else:
                        logger.warning(f"Failed to add Wikipedia image: {image_result.get('error', 'Unknown error')}")
    ...
    features = ["Synchronized captions"]
    if optimized_music:
        features.append("optimized background music")
    if wikipedia_image_path:
        features.append("Wikipedia page image")
